...
class EcommerceFeatureExtractor(BaseEstimator, TransformerMixin):
    ...
    Median-based features are calculated inside fit() to prevent
...
def build_xgboost_pipeline(remove_redundant_features=False):
    numeric_features, categorical_features = get_feature_types(
        remove_redundant_features=remove_redundant_features
    )

    # XGBoost is scale-invariant — no StandardScaler needed
...
print("\n" + "=" * 70)
print("XGBoost Experiment 1 — Baseline")
print("=" * 70)

model = build_xgboost_pipeline(remove_redundant_features=False)
...
# Out-of-fold predictions for aggregated metrics
...
try:
    ...
    IMPORTANCE_FILE = BASE_DIR / "xgboost_experiment_1_feature_importance.csv"
...
summary_df = pd.DataFrame([summary], index=["xgboost_experiment_1"])
