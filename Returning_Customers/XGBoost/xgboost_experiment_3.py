...
print("\n" + "=" * 70)
print("XGBoost Experiment 3 — RandomizedSearchCV")
print("=" * 70)
...
param_dist = {
    ...
    "classifier__gamma": [0, 0.1, 0.3, 0.5, 1.0],
    "classifier__scale_pos_weight": [1.0, scale_pos_weight, 1.5, 2.0],
}
...
print(f"\nBest F1 Macro (inner CV): {search.best_score_:.4f}")


# =========================================================
# 8. Evaluate best model with outer CV
# =========================================================

print("\n" + "-" * 70)
print("Evaluating best model with outer 5-Fold CV")
print("-" * 70)
...
cv_results = cross_validate(
    best_model, X_raw, y,
    cv=outer_cv, scoring=scoring, n_jobs=1, return_train_score=False
)
...
y_pred_cv = cross_val_predict(best_model, X_raw, y, cv=outer_cv, method="predict", n_jobs=1)
y_proba_cv = cross_val_predict(best_model, X_raw, y, cv=outer_cv, method="predict_proba", n_jobs=1)[:, 1]
...
try:
    ...
    IMPORTANCE_FILE = BASE_DIR / "xgboost_experiment_3_feature_importance.csv"
...
if SUMMARY_FILE.exists():
    ...
    new_row = pd.DataFrame([summary], index=["xgboost_experiment_3"])
    combined = pd.concat([existing, new_row])
    combined.to_csv(SUMMARY_FILE, index=True)
else:
    pd.DataFrame([summary], index=["xgboost_experiment_3"]).to_csv(SUMMARY_FILE, index=True)

print("\nBest params summary:")
best_params_df = pd.DataFrame([search.best_params_], index=["xgboost_experiment_3_best_params"])
best_params_df.to_csv(BASE_DIR / "xgboost_experiment_3_best_params.csv", index=True)
