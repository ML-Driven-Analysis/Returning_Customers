...
### ניסוי 3: Cross-Validation 5-Fold

**קובץ:** `logistic_experiment_3.py`

**מה עשינו:** הרצנו את שלושת ההגדרות (baseline / balanced / balanced+הסרת פיצ'רים) דרך **StratifiedKFold** עם 5 קפלים. כל קפל אומן על 4,000 רשומות ונבדק על 1,000 — ותוצאות הניבוי מחוץ לקפל (OOF) מצטברות לכלל הנתונים.

**מדוע CV עדיף על חלוקה חד-פעמית:**
חלוקת Train/Test אחת תלויה בהגרלה ועלולה לתת תוצאות אופטימיסטיות או פסימיסטיות. CV מייצר אומדן ממוצע על 5 חלוקות שונות — יציב ואמין יותר.

**טבלת השוואה — ממוצעי 5 קפלים:**

| מדד | Baseline | Balanced + כל הפיצ'רים | Balanced + הסרת פיצ'רים |
|---|---|---|---|
| Accuracy | 59.3% | 49.4% | 49.3% |
| Balanced Accuracy | 50.0% | 49.1% | 49.0% |
| Recall (קלאס 0) | 2.7% | 47.9% | 47.8% |
| Recall (קלאס 1) | 97.4% | 50.4% | 50.3% |
| F1 Macro | 0.396 | **0.487** | 0.486 |
| ROC AUC | 0.492 | **0.493** | 0.491 |

**מטריצות בלבול מצטברות (OOF על כל 5,000 רשומות):**
...
*Balanced + כל הפיצ'רים (המודל הטוב ביותר):*
...
1. **ROC AUC ≈ 0.49 לכל ההגדרות** — תוצאת ה-0.475 בחלוקה החד-פעמית הייתה פסימיסטית מעט; האומדן האמיתי קרוב יותר ל-0.49. עם זאת, כל ההגדרות מרוכזות באותה רמה — הפיצ'רים חלשים ב**כל** הגדרה.

2. **המודל הטוב ביותר: `balanced_all_features`** (F1 Macro 0.487, ROC AUC 0.493) — הסרת הפיצ'רים המקורלים לא שיפרה את הביצועים ב-CV; ה-5 פיצ'רים ה"מיותרים" מוסיפים רעש אך גם מעט אינפורמציה.

3. **CV מאשר: הבעיה היא בפיצ'רים** — גם עם 5 חלוקות שונות ROC AUC ≈ 0.49, כלומר אין מצב בו המודל הלינארי "עובד" על הנתונים האלה. המסקנה הסופית: **יש לנסות מודלים לא-לינאריים** (Random Forest / XGBoost).
