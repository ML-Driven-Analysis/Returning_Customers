"""
Churn Customer — XGBoost Experiment 2 (with held-out test set)

Same improvements as xgboost_experiment_2.py (feature engineering +
threshold tuning), but with a proper train/test split: 70% train,
30% test (stratified, random_state=42).

The test set is held out completely — the 5-fold CV and threshold
tuning run on TRAIN only. TEST is touched exactly once, at the end,
to report an unbiased final estimate.

scale_pos_weight is computed from TRAIN only.
...
print("=" * 70)
print("Churn Customer - XGBoost Experiment 2 (Feature Engineering + Threshold Tuning + Holdout)")
print("=" * 70)
...
scale_pos_weight = round(n0_train / n1_train, 2)

print("\n" + "=" * 70)
print("Train / Test split (70% / 30%, stratified)")
print("=" * 70)
print(f"  Train: {X_train.shape[0]:,} rows  "
      f"(Churn=1: {n1_train:,} | {(y_train == 1).mean()*100:.1f}%)")
print(f"  Test:  {X_test.shape[0]:,} rows  "
      f"(Churn=1: {(y_test == 1).sum():,} | {(y_test == 1).mean()*100:.1f}%)")
print(f"  scale_pos_weight (from TRAIN only): {scale_pos_weight}")
...
# =========================================================
# 8. Cross-validation - default threshold, TRAIN only
# =========================================================

print("\n" + "=" * 70)
print("Step 1: 5-Fold CV on TRAIN - default threshold=0.5...")
print("=" * 70)
...
# =========================================================
# 9. Threshold tuning - maximise F1 Macro (TRAIN OOF only)
# =========================================================

print("\n" + "=" * 70)
print("Step 2: Threshold tuning via Precision-Recall curve (TRAIN OOF)...")
print("=" * 70)
...
train_results_tuned = evaluate_model_cv(model, X_train, y_train, cv, thresholds=[best_threshold])
print_evaluation(train_results_tuned, label=f"Train CV (5-Fold, threshold={best_threshold:.4f})")


# =========================================================
# 10. Threshold sweep (TRAIN OOF only)
# =========================================================

print("\nThreshold sweep on TRAIN OOF (Recall vs Precision tradeoff):")
print(f"{'Threshold':>10} {'Precision(1)':>13} {'Recall(1)':>10} {'F1(1)':>8} {'F1 Macro':>10}")

for t in np.arange(0.1, 0.91, 0.05):
    y_t = (y_proba_train_oof >= t).astype(int)
    p1 = precision_score(y_train, y_t, pos_label=1, zero_division=0)
    r1 = recall_score(y_train, y_t, pos_label=1, zero_division=0)
    f1 = f1_score(y_train, y_t, pos_label=1, zero_division=0)
    fm = f1_score(y_train, y_t, average="macro", zero_division=0)
...
summary = {
    ...
    "train_balanced_accuracy_default": round(m_train_default["balanced_accuracy"], 6),
    ...
    "train_pr_auc": round(m_train_default["pr_auc"], 6),
    # train, optimal threshold
    "train_accuracy_tuned": round(m_train_tuned["accuracy"], 6),
    "train_balanced_accuracy_tuned": round(m_train_tuned["balanced_accuracy"], 6),
    ...
    "test_balanced_accuracy_default": round(m_test_default["balanced_accuracy"], 6),
    ...
    "test_pr_auc": round(m_test_default["pr_auc"], 6),
    # test, optimal threshold
    "test_accuracy_tuned": round(m_test_tuned["accuracy"], 6),
    "test_balanced_accuracy_tuned": round(m_test_tuned["balanced_accuracy"], 6),
...
SUMMARY_FILE = BASE_DIR / "xgboost_experiment_2_holdout_summary.csv"
pd.DataFrame([summary], index=["xgboost_experiment_2_holdout"]).to_csv(SUMMARY_FILE, index=True)
