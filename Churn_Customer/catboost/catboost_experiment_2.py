...
Changes vs Experiment 1:
  - auto_class_weights="Balanced"  → penalizes churn=1 misses more
  - Threshold tuning: evaluate at [0.3, 0.4, 0.5]

Split     : Hold-out 70% train / 30% test, stratified random split
...
print("Split: Hold-out 70/30, stratified random split")
print("=" * 70)
print(f"\nDataset: {DATA_FILE}")
...
# ===========================================================================
# 3. Split — 70/30 stratified random hold-out
# ===========================================================================
...
categorical_transformer = Pipeline(steps=[
    ...
    ("to_str", FunctionTransformer(lambda arr: arr.astype(str), feature_names_out="one-to-one")),
])
...
print("\n" + "=" * 70)
print("Comparison vs Experiment 1")
print("=" * 70)

threshold_headers = " ".join(
    [f"{'Exp 2 t=' + str(round(t, 2)):>18}" for t in THRESHOLDS]
)

print(f"  {'Metric':<22} {'Exp 1':>8} {threshold_headers}")
print(f"  {'-' * 90}")
...
# ===========================================================================
# 7.5 Extract and Print CatBoost Feature Importance
# ===========================================================================
...
trained_cb = model.named_steps["classifier"]
importances = trained_cb.get_feature_importance()

# Feature order follows the ColumnTransformer output:
# numeric features first, followed by categorical features.
